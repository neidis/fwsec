...
// expo : 2^2l(T+1)
void pSig(struct FAS_BlindProof* eta,BIGNUM* N,int T , BIGNUM* B, BIGNUM* W) {
	...
	BN_rand_range(r,N);
	power_function(J,r,2*l*(T+1),N,NULL);
	...
	BIGNUM* temp = BN_new();
	BN_mod_exp(temp, B, eta->h, N, ctx);
	BN_mod_mul(eta->seta, r, temp,N, ctx);
	BN_free(temp);
	BN_free(r);
	BN_free(J);
	cout << "---------------------------pSig end-----------------------------" << endl;
...
void setup(int k, int L, int T, int n, struct FAS_PubKey* PK, struct FAS_PriKey* SK)
{
	...
	power_function(PK->U, s, EXPO, N, NULL); // set PK U
	...
	power_function(PK->D, C, n, N_prime, NULL); //  set PK D
	BN_mod_inverse(SK->v,two,four_phiN_prime,ctx);
	

	BIGNUM* R0 = BN_new();
	BIGNUM* W = BN_new();
	BN_rand_range(R0, N); // R0 <- Z*n
	BN_copy(SK->B_j_1,R0); // B0 = R0
	
	power_function(W,R0,2*l*(T+1),N,NULL); // W = R0^{2*l*(T+1)}

	BN_mod_exp(SK->E_j, PK->D, SK->v, N_prime, ctx); // set SK->E_j
	
	power_function(SK->S_t, s, 2 * l, N, NULL); // set SK S_t
	pSig(SK->eta_j_1, PK->N,PK->T, SK->B_j_1, W); // set SK eta_j_1
...
void Sig(struct FAS_PriKey* SK, string M, struct FAS_Sign* Sign) {
	if (SK->j > SK->T + 1 || SK->t_j >= SK->n) // 
		...
		BIGNUM* W = BN_new(); // W_j
		BIGNUM* temp = BN_new();
		BN_rand_range(R,SK->N); // random R
		power_function(temp,R,2*l*(SK->T+1),SK->N,NULL); // R^{2^{2l(T+1)}}
		BN_mod_mul(W, SK->eta_j_1->W, temp, SK->N, ctx);// W_j = W_j-1 * Y^r
...
//expo : 2^2l(T+1)
int pVer(struct FAS_BlindProof* BP, BIGNUM* N,int T) {
	...
	BN_mod_exp(t2, BP->W, BP->h, N, ctx); // W^h
	BN_mod_inverse(t3, t2, N, ctx); // (W^h)^-1
	power_function(t4,BP->seta,2*l*(T+1),N,NULL);
	BN_mod_mul(J, t4, t3, N, ctx);
...
int Ver(struct FAS_PubKey* PK, struct FAS_Sign* Sign, struct FAS_PriKey* SK, string M) {
	...
	print_BN(c,"c in Ver : ");
	if (pVer(Sign->pre_eta, PK->N,PK->T)) {
		if (pVer(Sign->post_eta, PK->N,PK->T)) {
			BIGNUM* hash = BN_new();
			...
			if (!BN_cmp(hash, PK->D)) {
				...
				BN_mod_mul(res2, res2, Sign->post_eta->W, PK->N, ctx); // W_j/W_(j-1)
...
{
	ofstream testfile("ARFAS_nOpt_2048_32768.txt");
	...
	testfile.close();
	printf("PK Key size : %d\n\n",BN_num_bytes(PK->N) + sizeof(PK->T) + sizeof(PK->n) + BN_num_bytes(PK->N_prime) + BN_num_bytes(PK->D)+BN_num_bytes(PK->U));
	printf("SK Key size : %d\n\n",BN_num_bytes(SK->N)+sizeof(SK->T)+sizeof(SK->n)+BN_num_bytes(SK->N_prime)+BN_num_bytes(SK->v)+sizeof(SK->j)+sizeof(SK->t_j)+BN_num_bytes(SK->E_j)+
	BN_num_bytes(SK->B_j_1)+BN_num_bytes(SK->S_t)+BN_num_bytes(SK->eta_j_1->W)+BN_num_bytes(SK->eta_j_1->seta)+BN_num_bytes(SK->eta_j_1->h));
	printf("Sign Key size : %d\n\n",sizeof(Sign->i)+sizeof(Sign->j)+BN_num_bytes(Sign->sigma)+BN_num_bytes(Sign->E)
	+BN_num_bytes(Sign->pre_eta->W)+BN_num_bytes(Sign->pre_eta->seta)+BN_num_bytes(Sign->pre_eta->h)
	+BN_num_bytes(Sign->post_eta->W)+BN_num_bytes(Sign->post_eta->seta)+BN_num_bytes(Sign->post_eta->h));
