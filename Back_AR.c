...
int verify(struct Identity* ID,const char* M,struct FwSecSigSign* SIGN,struct FwSecSigPubKey* VK,int j,struct FwSecSigPriKey* SK){
	BIGNUM* h1 = BN_new();
	BIGNUM* h2 = BN_new();
...
int main(int argc, const char* argv[])
{
	...
	printf("Sign start\n");
	Sign(ID,1,SK,message,SIGN);
	printf("Sign ended\n");

	int flag;
	printf("Verify start\n");
	flag = verify(ID,message,SIGN,VK,1,SK);
	printf("verify ended\n");
