...
int fastSetup()
{
	...
	// use phi(N)
	BIGNUM* phi = BN_new();
	BN_sub(t1, rsa->p, BN_value_one());
	BN_sub(t2, rsa->q, BN_value_one());
...
void signMessage(struct FwSecSigSign* sign, BIGNUM* m)
{
	...
	//BIGNUM* t4 = BN_new();
        ...
        // k <- {0,1}^k
        BIGNUM* r = BN_new();
        BN_rand(r, gRSALength, 1, 1);
        //BN_rand(r, l, 1, 1);

        // R' <-- R^r
        BIGNUM* R = BN_new();
        BN_mod_exp(R, gPubKey->R, r, gPubKey->N, ctx);

	// Y' <-- Y^r
        BIGNUM* Y = BN_new();
        BN_mod_exp(Y, gPubKey->Y, r, gPubKey->N, ctx);
