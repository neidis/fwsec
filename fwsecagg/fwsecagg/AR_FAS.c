...
void mul_exp(BIGNUM* sign, BIGNUM* R, BIGNUM* S, char* C)
{
	BIGNUM* t1 = BN_new();
	BN_bin2bn(C,l/8, t1);
	BN_mod_exp(sign, S, t1, gPubKey->N, ctx);
...
int setup()
{
	...
	int i;
        ...
        // U <-- S^{2^{l(T+1)}}
        BN_mod_exp(gPubKey->U, gPriKey->S, t3, gPubKey->N ,ctx);
...
void init()
{
	...
	BN_mod_mul(gSessionKey->v, r_j, r_j0, gPubKey->N, ctx);
 	...
 	//exp_twos_power(gSessionKey->R, r_j, l*(gPubKey->T+1));
        BN_mod_exp(gSessionKey->R, r_j, two_exponent, gPubKey->N ,ctx);
