...
void generatePartialSign(struct FwSecAggSigPartSign *z, BIGNUM* v, int i)
{
	// w <- Z_N
	BIGNUM* w = BN_new();
	...
	// W <-- w^n
	exp_twos_power(z->W, w, l*(gPubKey->T+1));
...
//	BN_set_word(gPubKey->n, 7);
	...
	exp_twos_power(gPubKey->Y, gPriKey->x, l * (gPubKey->T + 1));
	//BN_mod_exp(gPubKey->Y, gPriKey->x, gPubKey->n, gPubKey->N, ctx);
	exp_twos_power(gPubKey->R, gPriKey->r, l * (gPubKey->T + 1));
	...
	generatePartialSign(gState, v, gPriKey->i);

	// key update (generate SK_1)
	// x^u
	//BN_mod_exp(t1, gPriKey->x, gPubKey->u, gPubKey->N, ctx);
	// x <-- x^{2^l}}
	exp_twos_power(t1, gPriKey->x, l);
	BN_copy(gPriKey->x, t1);

	// r^u 
	exp_twos_power(t1, gPriKey->r, l);
...
*/
	...
	// sigma <--
	hash_exp_mul(sign->sign, r, gPriKey->x, gPriKey->i, m, gPubKey->R);
...
*/
	...
	// z_i <--
	generatePartialSign(sign->z, v, gPriKey->i);
...
int verifyPartialSign(struct FwSecAggSigPartSign* sign, int index, struct FwSecAggSigPubKey* pubKey)
{
	...
	BIGNUM* t2= BN_new();
	exp_twos_power(t2, sign->sign, l*(pubKey->T+1));
...
int verify(struct FwSecAggSigSign* sign, BIGNUM** m, struct FwSecAggSigPubKey* pubKey)
{
	...
	exp_twos_power(t1, sign->sign, l*(pubKey->T+1));
	BN_mod_mul(left, t1, sign->z_1->V, pubKey->N, ctx);

	//  (RY^Hi^{2^l} (RY^H_{i+1})^{2^2l} ... (RY^H_{j-i+1})^{2^{l*(j-i+1}})^2^{l*(i-1)} 
	BIGNUM* acc = BN_new();
	BN_one(acc);

	int i;
	for(i=sign->finish; i>=sign->start; --i) {
		hash_exp_mul(t1, pubKey->R, pubKey->Y, i, m[i - sign->start], pubKey->R);
		BN_mod_mul(t2, acc, t1, pubKey->N, ctx);
		exp_twos_power(acc, t2, l);
	}

	// acc^u^i
	exp_twos_power(acc, t2, l*(sign->start));

	// V_i * acc
	BN_mod_mul(right, sign->z->V, acc, pubKey->N, ctx);
