...
{
	char hash_digest[SHA256_DIGEST_LENGTH];
	char temp[2048];
	SHA256_CTX sha_ctx;
	SHA256_Init(&sha_ctx);

	BN_bn2bin(a, temp);
	SHA256_Update(&sha_ctx, temp, BN_num_bytes(a));

	BN_bn2bin(b, temp);
	SHA256_Update(&sha_ctx, temp, BN_num_bytes(b));

	BN_bn2bin(c, temp);
	SHA256_Update(&sha_ctx, temp, BN_num_bytes(c));
...
//	for(i=0; i<gPubKey->T; i++) {
	...
	printf("PK size : %d\n",sizeof(gPubKey->T) + BN_num_bytes(gPubKey->U[0])*256 +BN_num_bytes(gPubKey->N));
	printf("SK size : %d\n",sizeof(gPriKey->i) + BN_num_bytes(gPriKey->S[0])*256);
